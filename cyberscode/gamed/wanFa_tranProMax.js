const wanFa_tranProMax = {
	tmid: null,
	//游戏启动控制部分
	gameStart(difficulty, scoreC) {
		//添加监听
		engine.addEventListener("gamePlay", wanFa_tranProMax.gamePlay);
		engine.addEventListener("gameTimeOver", wanFa_tranProMax.gameTimeOver);
		engine.addEventListener("gameDestroy", wanFa_tranProMax.gameDestroy);
		//重置全局变量
		tranProFuncs.resetAll()
		clearInterval(tranProFuncs.CountPlay.innerCount)
		...
		let countTime = 0
		wanFa_tranProMax.gameStart.startLoop = setInterval(() => {
			switch (countTime) {
				case 1:
				...
				case 16:
					if (levelInfos.wanFa.startsWith("tranProMax")) {
						...
						tranProFuncs.CountPlay();
					}
					break;
				case 17:
					clearInterval(wanFa_tranProMax.gameStart.startLoop)
					break;
	...
	//游戏开始
	gamePlay(gameid) {
		engine.addEventListener("gameTaped", wanFa_tranProMax.gameTaped)
		engine.log("添加tap监听")
		engine.log("-------------------" + gameid)
		engine.log(canTapNum + "???")
		if (gameid.startsWith("transProMax")) {
			clearInterval(tranProFuncs.tiaoWin.Athree)
			clearNum = 0
			tranProFuncs.blueNoTap()
			nowInfos.target = ((usersInfos.usersResult.length) * 3) + 1;
			nowInfos.nowGameid = gameid;
			gameRules.lifeMove();
			//roomFunction.playSound(false, "togreen")
			tranProFuncs.CountPlay()
			tranProFuncs.ScorePlay()

			//这里
			tranProFuncs.blueFor()
			tranProFuncs.blueShow(usersInfos.usersResult.length)
			tranInfos.biepeng = 0
			...
			if (guole == 0) {
				...
				tranProFuncs.allblack()
				setTimeout(() => {
					if (gameid == "transProMax-cxx") {
						tranProFuncs.addRedJiaoxue1()
						tranProFuncs.addRedJiaoxue2()
						tranProFuncs.redPlayJiaoxue1()
						tranProFuncs.redPlayJiaoxue2()


					} else if (gameid == "transProMax03-cxx") {
						tranProFuncs.addRed()
						tranProFuncs.addRed1()
						tranProFuncs.redPlay()
						tranProFuncs.redPlay1()
					} else if (gameid == "transProMax02-cxx") {

						tranProFuncs.addRed()
						tranProFuncs.redPlay()
					} else if (gameid == "transProMax04-cxx") {
						tranProFuncs.redPlay13()
					}

					tranProFuncs.blueYesTap()
					switch (guole) {
						case 0:
							usersInfos.levelScore = 0;
							tranProFuncs.addRed8()
				...
				setTimeout(() => {
					...
					if (gameid == "transProMax-cxx") {
						...
						tranProFuncs.redPlay13()
					} else if (gameid == "transProMax05-cxx") {
						...
						tranProFuncs.redPlay5()
					} else if (gameid == "transProMax07-cxx") {
						tranProFuncs.redPlay6()
					} else if (gameid == "transProMax08-cxx") {
						tranProFuncs.redPlay7()
					} else if (gameid == "transProMax09-cxx") {
						tranProFuncs.redPlay8()
					}
					else if (gameid == "transProMax10-cxx") {
						//tranFuncs.redPlay8()
						tranProFuncs.addRed()
						tranProFuncs.addRed1()
						tranProFuncs.redPlay10()
						tranProFuncs.redPlay11()
					}
					tranProFuncs.blueYesTap()
					switch (guole) {
						case 0:
	...
	gameTaped(face, x, y, onOff, nodeId, event) {
		...
		if (nowInfos.nowGameid.startsWith("transProMax") && tranInfos.biepeng == 0) {
			...
			tranProFuncs.blueTap(face, x, y, onOff, nodeId, event);
			tranProFuncs.tapWrong(face, x, y, onOff, nodeId, event);
			tranProFuncs.gameEndControl(face, x, y, onOff, nodeId, event)
			...
			if (onOff == false && deferredInfos.xHistoricalData == x && deferredInfos.yHistoricalData == y) {
				...
				clearTimeout(tranProFuncs.tapWrong.determine)
			}
	...
	gameDestroy(gameid) {
		...
		clearInterval(tranProFuncs.CountPlay.innerCount)
		engine.removeEventListener("gameTaped", wanFa_tranProMax.gameTaped)
		engine.log("移除tap监听")
	},
	...
	gameLevelEnd() {
		...
		clearInterval(tranProFuncs.CountPlay.innerCount)
		clearInterval(wanFa_tranProMax.gamePlay.count)
		clearInterval(tranProFuncs.tiaoWin.Athree)
		clearTimeout(tranProFuncs.blueShow.colorClear)
...
const tranProFuncs = {
	...
	gameEndControl(face, x, y, onOff, nodeId, event) {
		if (nodeId == "StopRed" && face == "a" && x == 0 && y == 0 && onOff == true) {
			engine.log("开始定时器")
			tranProFuncs.gameEndControl.haha = setTimeout(() => {
				wanFa_tranProMax.gameLevelEnd()
			}, 10000);
		}
		if (nodeId == "StopRed" && face == "a" && x == 0 && y == 0 && onOff == false) {
			engine.log("清除定时器")
			clearTimeout(tranProFuncs.gameEndControl.haha)
		}
	},
	//数字1
	addRed8() {
	...
	allblack() {
		...
		gameFuncs.op(opInfo)
		setTimeout(() => {
			fastop.removeNode("nmd", "black0089")
		}, 8000);
	...
	blueShow(num) {
		...
		while (canTapNum.length < num) {
			if (nowInfos.target !== 1 && xianshi == 1) {
	...
	blueReveal() {
		for (let i = 0; i < canTapNum.length; i++) {
			...
			gameFuncs.op(opInfo2);
			tranProFuncs.blueShow.colorClear = setTimeout(() => {
	...
	rmAllListener() {
		...
		engine.removeEventListener("gamePlay", wanFa_tranProMax.gamePlay)
		engine.removeEventListener("gameTaped", wanFa_tranProMax.gameTaped)
		engine.removeEventListener("gameDestroy", wanFa_tranProMax.gameDestroy)
		engine.removeEventListener("gameTimeOver", wanFa_tranProMax.gameTimeOver)
	},
	...
	tapWrong(face, x, y, onOff, nodeId, event) {
		if (nowInfos.lifeProtect == 0) {
			if (nodeId.startsWith("red") && onOff == true && cuowu == 0) {
				...
				// setTimeout(() => {
				// 	tranProFuncs.addBlink(x, y)
				// }, 200);
				nowInfos.lifeProtect = 1;
				setTimeout(function () {
					nowInfos.lifeProtect = 0
				}, 1000);
				tranProFuncs.tapWrong.determine = setTimeout(() => {
					tranProFuncs.addBlink(x, y)
					usersInfos.UseLife++
					if (nowInfos.lifePoint > 1) {
						...
						tranProFuncs.allblack()
						setTimeout(() => {
							tranProFuncs.addRed70() // lose
						}, 500);
						tranProFuncs.tapWrong.awa = setTimeout(() => {
							if (nowInfos.gameCountTime <= 0) {
								clearTimeout(tranProFuncs.tapWrong.awa)
								return
							}
							tranProFuncs.tiaoWin()
							nowInfos.lifePoint = 6
							gameRules.lifeMove();
							cuowu = 0
	...
	tiaoWin() {
		...
		tranProFuncs.addRed50()
		tranProFuncs.tiaoWin.Athree = setInterval(() => {
			clearNum++
			switch (clearNum) {
				case 1:
					tranProFuncs.oneFor()
					tranProFuncs.addRed60()
					break;
				case 2:
					tranProFuncs.oneFor1()
					tranProFuncs.addRed90()
					break;
				case 3:
					tranProFuncs.oneFor90()
					tranProFuncs.addRed30()
					tranProFuncs.addRed40()
					break;
			}
		}, 1000);
		...
		// tranProFuncs.tiaoWin.three = setTimeout(() => {
		// 	// fastop.removeNode("ccc", "one222")
		// 	tranProFuncs.oneFor()
		// 	tranProFuncs.addRed60()

		// }, 1000);
		// tranProFuncs.tiaoWin.two = setTimeout(() => {
		// 	tranProFuncs.oneFor1()
		// 	tranProFuncs.addRed90()
		// }, 2000);
		// tranProFuncs.tiaoWin.one = setTimeout(() => {
	...
	// 	roomFunction.playSoundTivite(false, "321Go", "positive")
	// 	tranProFuncs.addRed50()
	// 	tranProFuncs.tiaoWin20.three1 = setTimeout(() => {
	// 		// fastop.removeNode("ccc", "one222")
	// 		tranProFuncs.oneFor()
	// 		tranProFuncs.addRed60()

	// 	}, 1000);
	// 	tranProFuncs.tiaoWin20.two1 = setTimeout(() => {
	// 		tranProFuncs.oneFor1()
	// 		tranProFuncs.addRed90()
	// 	}, 2000);
	// 	tranProFuncs.tiaoWin20.one1 = setTimeout(() => {
	// 		// fastop.removeNode("ccc", "one223")
	// 		tranProFuncs.oneFor90()
	// 		tranProFuncs.addRed30()
	// 		tranProFuncs.addRed40()
	...
	CountPlay() {
		tranProFuncs.CountPlay.innerCount = setIntervalCount(function (index, count) {
			let opInfo = {
