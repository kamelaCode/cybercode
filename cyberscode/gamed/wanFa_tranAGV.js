const wanFa_tranAGV = {
	...
	gameStart(difficulty, scoreC) {
		//添加监听
		engine.addEventListener("gamePlay", wanFa_tranAGV.gamePlay);
		engine.addEventListener("gameTimeOver", wanFa_tranAGV.gameTimeOver);
		engine.addEventListener("gameDestroy", wanFa_tranAGV.gameDestroy);
		//重置全局变量
		tranAGVFuncs.resetAll()
		clearInterval(tranAGVFuncs.CountPlay.innerCount)
	...
	//游戏开始
	gamePlay(gameid) {
		engine.addEventListener("gameTaped", wanFa_tranAGV.gameTaped)
		engine.log("添加tap监听")
		engine.log("-------------------" + gameid)
		...
		if (gameid.startsWith("trans")) {
			...
			tranAGVFuncs.CountPlay()
			tranAGVFuncs.ScorePlay()
			tranAGVFuncs.blueFor()
			if (zwy == 0) {
				roomFunction.playSound(false, "gotoGreen")
			}
			tranAGVFuncs.blueShow(usersInfos.usersResult.length)
			tranAGVFuncs.blueNoTap()
			setTimeout(() => {
				if (levelInfos.level != 5) {
					clearInterval(hahaha)
				}
				//tranAGVFuncs.greenRm()
				roomFunction.playSound(false, "tranRules")
				wanFa_tranAGV.gamePlay.count = setTimeout(() => {
					roomFunction.playSound(true, "tranBgm", "background")
				}, 11000);
				if (levelInfos.level == 1) {
					tranAGVFuncs.addRedJiaoxue1()
					tranAGVFuncs.addRedJiaoxue2()
					tranAGVFuncs.redPlayJiaoxue1()
					tranAGVFuncs.redPlayJiaoxue2()


				} else if (levelInfos.level == 2) {
					//fastop.nodeMove("redMove", "red301", 3, true, "b", 0, 0, 0, 0, 0, 30)
					tranAGVFuncs.addRed()
					tranAGVFuncs.addRed1()
					tranAGVFuncs.redPlay()
					tranAGVFuncs.redPlay1()
				} else if (levelInfos.level == 3) {

					tranAGVFuncs.addRed()
					tranAGVFuncs.redPlay()
				} else if (levelInfos.level == 4) {
					tranAGVFuncs.addRed12()
					tranAGVFuncs.redPlay13()
					...
					tranAGVFuncs.redPlay5()
				} else if (levelInfos.level == 7) {
					tranAGVFuncs.redPlay6()
				} else if (levelInfos.level == 8) {
					tranAGVFuncs.redPlay7()
				} else if (levelInfos.level == 9) {
					tranAGVFuncs.redPlay8()
				}
				else if (levelInfos.level == 10) {
					//tranAGVFuncs.redPlay8()
					tranAGVFuncs.addRed()
					tranAGVFuncs.addRed1()
					tranAGVFuncs.redPlay10()
					tranAGVFuncs.redPlay11()
				}
				tranAGVFuncs.blueYesTap()
			}, 6000);
	...
	gameTaped(face, x, y, onOff, nodeId, event) {
		...
		if (nowInfos.nowGameid.startsWith("trans")) {
			...
			tranAGVFuncs.blueTap(face, x, y, onOff, nodeId, event);
			tranAGVFuncs.greenTap(face, x, y, onOff, nodeId, event);
			tranAGVFuncs.tapWrong(face, x, y, onOff, nodeId, event);
			...
			if (onOff == false && deferredInfos.xHistoricalData == x && deferredInfos.yHistoricalData == y) {
				...
				clearTimeout(tranAGVFuncs.tapWrong.determine)
			}
	...
	gameDestroy(gameid) {
		...
		clearInterval(tranAGVFuncs.CountPlay.innerCount)
		engine.removeEventListener("gameTaped", wanFa_tranAGV.gameTaped)
		engine.log("移除tap监听")
	},
	...
	gameLevelEnd() {
		...
		clearInterval(tranAGVFuncs.CountPlay.innerCount)
		clearInterval(wanFa_tranAGV.gamePlay.count)
		...
		tranAGVFuncs.rmAllListener()
		wanFaCtl_tranAGVCtl.gameEndCtl(usersInfos.levelScore, nowInfos.lifePoint)
...
const tranAGVFuncs = {
	...
	blueShow(num) {
		...
		tranAGVFuncs.blueReveal()
		// }
	},
	//控制目标点位闪烁
	blueReveal() {
		//if (nowInfos.target == 20) {
		engine.log("是这里")
		let opInfo9 = {
			opId: "blue" + canTapNum[0],
			opType: "play",
		...
		// } else if (nowInfos.target != 20) {
		// 	engine.log("在这呢")
		// 	let opInfo3 = {
		// 		opId: "blue" + canTapNum[canTapNum.length - 1],
		// 		opType: "play",
		// 		opNode: "blue00" + canTapNum[canTapNum.length - 1],
		// 		timeLen: 1,
		// 		loop: "true",
		// 		keyFrames: [
		// 			{
		...
		// if (nowInfos.target < 20) {
		// 	let opInfo4 = {
		// 		opId: "green" + canTapNum[1],
		// 		opType: "play",
		// 		opNode: "green00" + canTapNum[1],
		// 		timeLen: 2,
		// 		loop: "true",
		// 		keyFrames: [
		// 			{
	...
	rmAllListener() {
		...
		engine.removeEventListener("gamePlay", wanFa_tranAGV.gamePlay)
		engine.removeEventListener("gameTaped", wanFa_tranAGV.gameTaped)
		engine.removeEventListener("gameDestroy", wanFa_tranAGV.gameDestroy)
		engine.removeEventListener("gameTimeOver", wanFa_tranAGV.gameTimeOver)
	},
	...
	tapWrong(face, x, y, onOff, nodeId, event) {
		if (nowInfos.lifeProtect == 0) {
			if (nodeId.startsWith("red") && onOff == true) {
				...
				// setTimeout(() => {
				// 	tranAGVFuncs.addBlink(x, y)
				// }, 200);
				nowInfos.lifeProtect = 1;
				setTimeout(function () {
					nowInfos.lifeProtect = 0
				}, 1000);
				tranAGVFuncs.tapWrong.determine = setTimeout(() => {
					tranAGVFuncs.addBlink(x, y)
					usersInfos.UseLife++
	...
	blueTap(face, x, y, onOff, nodeId, event) {
		...
		if (nodeId == ("blue00" + canTapNum[0]) || nodeId == ("blue00" + canTapNum[canTapNum.length - 1]) && onOff == true && tranInfos.blueProject == 1) {
			...
			tranAGVFuncs.blueTap.fanfan = setTimeout(() => {
				nowInfos.lifePoint--;
				gameRules.lifeMove();
				roomFunction.playSoundTivite(false, "wrong", "negative");
	...
	greenTap(face, x, y, onOff, nodeId, event) {
		if (face == "b" && (nodeId == ("green00" + canTapNum[canTapNum.length - 1])) && onOff == true) {
			...
			clearTimeout(tranAGVFuncs.blueTap.fanfan)
		}
	},
	...
	CountPlay() {
		tranAGVFuncs.CountPlay.innerCount = setIntervalCount(function (index, count) {
			let opInfo = {
