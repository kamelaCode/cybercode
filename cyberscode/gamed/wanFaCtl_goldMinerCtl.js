const wanFaCtl_goldMinerCtl = {
	gameLevelCtl(mode, wanFa, level, gameIds) {
		if (mode == "competMode") {
			wanFa_goldMiner.gameStart(level, 10, mode, wanFa, level, gameIds)
		}
		...
		if (levelInfos.mode == "competMode") {
			...
			for (let i = 0; i < usersInfos.usersResult.length; i++) {
				switch (levelInfos.level) {
					case 1:
						usersInfos.usersResult[i].disVal = goldInfos.block2Infos[i] * 24;
						usersInfos.usersScore[i] = goldInfos.block2Infos[i] * 24
						sum += usersInfos.usersScore[i]
						break;
					case 2:
						usersInfos.usersResult[i].disVal = goldInfos.block2Infos[i] * 20;
						usersInfos.usersScore[i] = goldInfos.block2Infos[i] * 20
						sum += usersInfos.usersScore[i]
						break;
					case 3:
						usersInfos.usersResult[i].disVal = goldInfos.block2Infos[i] * 13;
						usersInfos.usersScore[i] = goldInfos.block2Infos[i] * 13
						sum += usersInfos.usersScore[i]
						break;
