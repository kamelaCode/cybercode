const wanFaCtl_lootCtl = {
	gameLevelCtl(mode, wanFa, level, gameIds) {
		if (mode == "competMode") {
			wanFa_loot.gameStart(level, 1, mode, wanFa, level, gameIds)
		}
	...
	gameEndCtl() {
		if (levelInfos.mode == "competMode") {
			...
			for (let i = 0; i < usersInfos.usersResult.length; i++) {
				usersInfos.usersResult[i].disVal = parseInt(playersScList[i] / 2);
				usersInfos.usersScore[i] = parseInt(playersScList[i] / 2)
