const wanFa_tran = {
	...
	gameStart(difficulty, scoreC) {
		//添加监听
		engine.addEventListener("gamePlay", wanFa_tran.gamePlay);
		engine.addEventListener("gameTimeOver", wanFa_tran.gameTimeOver);
		engine.addEventListener("gameDestroy", wanFa_tran.gameDestroy);
		//重置全局变量
		tranFuncs.resetAll()
		clearInterval(tranFuncs.CountPlay.innerCount)
		...
		let countTime = 0
		wanFa_tran.gameStart.startLoop = setInterval(() => {
			switch (countTime) {
				case 1:
				...
				case 16:
					...
					break;
				case 17:
					clearInterval(wanFa_tran.gameStart.startLoop)
					break;
	...
	//游戏开始
	gamePlay(gameid) {
		engine.addEventListener("gameTaped", wanFa_tran.gameTaped)
		engine.log("添加tap监听")
		engine.log("-------------------" + gameid)
		...
		if (gameid.startsWith("trans")) {
			...
			tranFuncs.blueShow(usersInfos.usersResult.length)
			tranFuncs.blueNoTap()
			setTimeout(() => {
				...
				wanFa_tran.gamePlay.count = setTimeout(() => {
					roomFunction.playSound(true, "tranBgm", "background")
				}, 7000);
				if (levelInfos.level == 1) {
					tranFuncs.addRedJiaoxue1()
					tranFuncs.addRedJiaoxue2()
					tranFuncs.redPlayJiaoxue1()
					tranFuncs.redPlayJiaoxue2()
	...
	gameTaped(face, x, y, onOff, nodeId, event) {
		...
		if (nowInfos.nowGameid.startsWith("trans")) {
			...
			tranFuncs.blueTap(face, x, y, onOff, nodeId, event);
			tranFuncs.greenTap(face, x, y, onOff, nodeId, event);
			tranFuncs.tapWrong(face, x, y, onOff, nodeId, event);
			...
			if (onOff == false && deferredInfos.xHistoricalData == x && deferredInfos.yHistoricalData == y) {
				...
				clearTimeout(tranFuncs.tapWrong.determine)
			}
	...
	gameDestroy(gameid) {
		...
		clearInterval(tranFuncs.CountPlay.innerCount)
		engine.removeEventListener("gameTaped", wanFa_tran.gameTaped)
		engine.log("移除tap监听")
	},
	...
	gameLevelEnd() {
		...
		clearInterval(tranFuncs.CountPlay.innerCount)
		clearInterval(wanFa_tran.gamePlay.count)
...
const tranFuncs = {
	...
	rmAllListener() {
		...
		engine.removeEventListener("gamePlay", wanFa_tran.gamePlay)
		engine.removeEventListener("gameTaped", wanFa_tran.gameTaped)
		engine.removeEventListener("gameDestroy", wanFa_tran.gameDestroy)
		engine.removeEventListener("gameTimeOver", wanFa_tran.gameTimeOver)
	},
	...
	tapWrong(face, x, y, onOff, nodeId, event) {
		if (nowInfos.lifeProtect == 0) {
			if (nodeId.startsWith("red") && onOff == true) {
				...
				// setTimeout(() => {
				// 	tranFuncs.addBlink(x, y)
				// }, 200);
				nowInfos.lifeProtect = 1;
				setTimeout(function () {
					nowInfos.lifeProtect = 0
				}, 1000);
				tranFuncs.tapWrong.determine = setTimeout(() => {
					tranFuncs.addBlink(x, y)
					usersInfos.UseLife++
	...
	CountPlay() {
		tranFuncs.CountPlay.innerCount = setIntervalCount(function (index, count) {
			let opInfo = {
